...
def evaluate(val_loader, model, criterion, epoch, log, writer):
    ...
    for i, sample_batched in enumerate(val_loader):
        ...
        # feed in network
        output, latent_mu, latent_logvar = model(x)
        ...
        # visuaulzai
        if epoch % 20 == 0:
            output_ = vutils.make_grid(output,
                                       normalize=False,
                                       scale_each=True)
            images_ = vutils.make_grid(x, normalize=False, scale_each=True)
            label_ = vutils.make_grid(y, normalize=False, scale_each=True)
